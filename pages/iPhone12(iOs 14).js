...
<path d="M62 38l-7.259-9.86L46 36"></path>
  ...
  $(".widget.water").on("click", function (e) {
    $(".mainScreen").addClass("blur");
    $(".water_screen.hidden").removeClass("hidden");
  });

  $(".widget.breathe").on("click", function (e) {
    $(".mainScreen").addClass("blur");
    $(".breathe_screen.hidden").removeClass("hidden");

    setTimeout(function () {
      $(".lockScreen").addClass("hidden");
      $(".breathe_screen").addClass("hidden");
      $(".appScreen.hidden").removeClass("hidden");
    }, 20000);
  });

  $("#angry, #sad, #puzzled, #okay, #happy").on("click", function (e) {
    var id = $(this).attr("id");
    var element = document.getElementById(id);

    if (element.style.boxShadow) {
      document.getElementById(id).style.removeProperty("box-shadow");
    } else {
      document
        .getElementById(id)
        .style.setProperty("box-shadow", "inset 0 0 0 1000px rgba(0,0,0,.6)");
  ...
  $(".widget.reflect").on("click", function (e) {
    $(".mainScreen").addClass("blur");
    $(".reflect_screen.hidden").removeClass("hidden");
  });

  $(".widget.breaktime").on("click", function (e) {
    $(".lockScreen").addClass("hidden");
    $(".appScreen.hidden").removeClass("hidden");
  });

  $(".widget_two.todo_widget").on("click", function (e) {
    $(".mainScreen").addClass("blur");
    $(".todo_screen.hidden").removeClass("hidden");
  });

  $(".reflect_screen").touchMov({
    mov: "y",
    movUp: function (e) {
      $(e.currentTarget).addClass("hidden");
  ...
  $(".minus_image").on("click", function (e) {
    if (
      +$(".current .number").text() > 0 &&
      +$(".current .number").text() <= 3.5
    ) {
      var waterNumber = +$(".current .number").text() - 0.25;
      $(".current .number").text(waterNumber);
      $(".highlight").text(waterNumber);
      var trackerHeight = $(".tracker").position().top;
      $(".tracker").css("top", trackerHeight + 27);
      var sliderHeight = $(".slider").position().top;
      $(".slider").css("top", sliderHeight + 27);
      var trackerSteps = $(".tracker .steps").position().top;
      $(".tracker .steps").css("top", trackerSteps - 27);
    }
    if (+$(".current .number").text() > 3.5) {
      var waterNumber = +$(".current .number").text() - 0.25;
      $(".current .number").text(waterNumber);
      $(".highlight").text(waterNumber);
    }
  ...
  $(".plus_image").on("click", function (e) {
    if (+$(".current .number").text() < 3.5) {
      var waterNumber = +$(".current .number").text() + 0.25;
      $(".current .number").text(waterNumber);
      $(".highlight").text(waterNumber);
  ...
  $(".fa.fa-check").click(function () {
    var parentItem = $(this).parent();
    var check = $("#" + $(parentItem).attr("for"));

    let checkbox = document.getElementById($(parentItem).attr("for"));
    if (!checkbox.checked) {
      setTimeout(function () {
        $(".lockScreen").addClass("hidden");
        $(".todo_screen").addClass("hidden");
  ...
  $("#submit").on("click", function (e) {
    ...
    setTimeout(function () {
      $(".lockScreen").addClass("hidden");
      $(".reflect_screen").addClass("hidden");
      $(".appScreen.hidden").removeClass("hidden");
    }, 1000);
  });
    ...
    let timeout = setTimeout(() => {
      ...
      if (!globalState.draggScreen) {
        if ($(e.target).hasClass("app") || $(e.target).parents(".app").length) {
          ...
          let cssMenu = `left: ${
            rectsIphone.x + rectsIphone.width - rectsApp.x >= 190
              ? rectsApp.x
              : rectsApp.x + rectsApp.width - 190
          }px;`;
          if (rectsIphone.top + 65 * 2 >= rectsApp.top) {
            cssMenu += `top : ${
              rectsApp.y + rectsApp.height
...
`);
  ...
  $("body").on("click", ".fixedMenuFixedApp .menuOption.remove", function () {
    ...
    alertiOS({
      headers: `Move " ${
        idApp !== undefined ? globalState.apps[idApp].name : "app"
      } " to App Library or delete the app?`,
      messages:
        "Moving the app will remove it from the Home screen and keep all its data",
      actions: [
        {
          text: "Delete app",
          warning: true,
        ...
        },
        {
          ...
          callback: function () {
            console.log("Pending Apps Library");
          },
        },
        {
          text: "Cancel",
...
function myfunc() {
  ...
  $(".lockScreen.hidden").removeClass("hidden");
  $(".appScreen").parent().removeClass("blur");
}
...
var guestMessages = {
  ...
  getMessages: function (msg) {
    ...
    today = mm + "/" + dd + "/" + yyyy;
    this.msgs.push(today + ": " + value + "<br />" + "<br />");
  },

  printMessages: function () {
    guestMessages.getMessages();
    var showEntry = (document.getElementById("first-entry").innerHTML =
      this.msgs);
  },
};
